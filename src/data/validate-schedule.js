// Reference validation script for fixture schedule correctness
// Run: node src/data/validate-schedule.js
// Cross-references all 72 fixtures against official FIFA schedule data

import { FIXTURE_SCHEDULE } from './schedule.js';
...
for (const [id, correct] of Object.entries(CORRECT)) {
  const current = FIXTURE_SCHEDULE[id];
  if (!current) { console.error(`MISSING: ${id}`); errors++; continue; }
  const errs = [];
  if (current.date !== correct.date) errs.push(`date: "${current.date}" != "${correct.date}"`);
  if (current.timeET !== correct.timeET) errs.push(`timeET: "${current.timeET}" != "${correct.timeET}"`);
  if (current.timeCEST !== correct.timeCEST) errs.push(`timeCEST: "${current.timeCEST}" != "${correct.timeCEST}"`);
  if (current.venue !== correct.venue) errs.push(`venue: "${current.venue}" != "${correct.venue}"`);
  if (errs.length) { console.error(`ERR ${id}: ${errs.join(', ')}`); errors++; }
}

if (errors === 0) {
  console.log('✓ All 72 fixtures match the official FIFA schedule.');
} else {
  process.exit(1);
}
